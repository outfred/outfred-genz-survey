    ...
    <div className="mb-8 glass-panel rounded-xl p-6 shadow-sm transition-all hover:shadow-md animate-fadeIn">
      ...
      <div className="space-y-3">
        {question.options.map((option) => (
          <label
            key={option.id}
            className={`group flex cursor-pointer items-center rounded-lg border border-transparent p-3 transition-all hover:bg-secondary/50 ${
          ...
          >
            ...
            <span
              className={`ms-3 text-base ${
                selectedOption === option.id ? 'font-semibold text-foreground' : 'text-muted-foreground'
